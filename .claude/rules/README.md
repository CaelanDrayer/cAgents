...
`core/hooks.md` is the canonical source. Summary: 18 distinct hook event types registered in `.claude/settings.json`; 34 `.cjs` files under `.claude/hooks/` (26 unique registered hooks + 5 dispatched sub-validators run by `write-edit-dispatch.cjs` + `agent-dispatch.cjs` + `hook-utils.cjs` + `run-hook.cjs` launcher + `bash-guard-evaluator.cjs` library). All hooks use the `createHook()` factory. Events not currently used by cAgents (e.g., `WorktreeCreate`, `WorktreeRemove`, `CwdChanged`, `FileChanged`, `Elicitation`, `ElicitationResult`) remain available for user-defined hooks.
