...
31 .cjs files implementing 28 unique hooks across 17 event types via the `createHook()` factory. See @resources/hook-catalog.md for per-hook detail (purpose, matchers, inputs, outputs, side effects).

> **Deferred refactor (v12.7.0)**: the counts (31 / 28 / 17) are hardcoded in
...
> + a settings.json walker) would prevent the drift WI-7 fixed in v12.6.x.
> Tracked as low-priority — only useful once a future change shifts the counts
> again.
