...
createHook('TeamTaskComplete', async (input) => {
  ...
  let systemMsg = `Work item ${workItemId} completed by ${memberName}. Progress: ${completedCount}/${totalCount}.`;
  if (newlyUnblocked.length > 0) {
    systemMsg += ` Newly unblocked: ${newlyUnblocked.map(i => i.id).join(', ')}.`;
  }
  if (completedCount === totalCount) {
    systemMsg += ' All work items complete - ready for validation.';
  }

  return { continue: true, systemMessage: systemMsg };
});
