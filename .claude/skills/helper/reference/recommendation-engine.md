...
> _V11.0 removed `/review`, `/optimize`, `/context`, `/debug`; v12.1.2 folded `/improve` into `/run` via the keyword router. See [docs/MIGRATION-V11.md](../../../../docs/MIGRATION-V11.md). Review/audit intent now routes to `/run review` (or `/run audit`), optimize intent to `/run optimize`, review+optimize to `/run improve`. Context and debug intents continue to route to `/run context ...` and `/run --mode debug` (unchanged)._

## Keyword Router Discovery (v12.1.2+)

`/helper` uses the `/run` keyword router as a primary discovery mechanism
for review and optimization intent. In v12.1.2, the standalone `/improve`
skill was folded into `/run`. When `/run`'s first request token is one of
...
the infrastructure" recommendation `/helper` produces becomes a `/run X`
...
|---------|---------------|--------------------|
| `improve` | `full` (review-then-optimize, shared baseline) | `/run improve src/auth/` |
| `review` | `review` (3-group parallel specialist review) | `/run review src/api/ --focus security` |
| `audit` | `review` (alias for review) | `/run audit infrastructure --type infrastructure` |
| `optimize` | `optimize` (opportunity scan + ROI rank + atomic apply) | `/run optimize "Reduce bundle size"` |
...
1. **Intent → keyword**: When the recommendation engine classifies user
   ...
   the recommended invocation. The user types `/run <keyword> <request>`
   directly.
...
3. **Non-first-word safety**: Keywords matched only on the first token.
   `/run check the audit logs` does NOT trigger review mode — `check` is
...
4. **Pre-v12.1.2 surfaces**: Any `/improve --mode <X>` invocation seen
   in user history or old docs maps to `/run <X>`. /helper should
...
| Request history | 0.10 | If the user recently mentioned planning or design in the same session, boost `/designer` by 0.10. If they mentioned review or audit, boost `/run` (via the `review` or `audit` keyword router). Otherwise 0.00 for all. |
...
1. **package.json exists** -- engineering domain hint -- add +0.06 to `/run` (covers `/run review` / `/run optimize` via keyword router)
...
3. **Current git branch name** (run `git branch --show-current`):
   ...
   - `main`, `master`, `release/*` -- add +0.06 to `/run review` (diff-aware review hint via keyword router)
...
Signal 1 - Keyword match (0.30):
  ...
  No matches for /team, /designer (/run keyword router not engaged
...
Signal 2 - Project context (0.30):
  package.json found -> engineering hint -> /run +0.06
...
Recommendation: /run Build REST API for user management
  ...
  first token would trigger /run's keyword router (mode=review),
...
### Check / Review / Audit Intent -> /run review (or /run audit)
...
**Examples**:
- "Check my code for security issues" -> `/run review --focus security`
- "Review the pull request" -> `/run review --scope changed`
- "Audit our infrastructure for cost savings" -> `/run audit --type infrastructure`
- "Check if our docs are up to date" -> `/run review docs/ --type documentation`

> Note: `audit` is an alias for `review` in the keyword router. `/run audit <target>` and `/run review <target>` produce identical pipelines.

### Improve / Optimize / Speed Up Intent -> /run optimize
...
**Examples**:
- "Make the app faster" -> `/run optimize "Make the app faster"`
- "Reduce our bundle size" -> `/run optimize --type code --focus performance`
- "Improve our SEO rankings" -> `/run optimize --type content --focus quality`
- "Streamline the onboarding process" -> `/run optimize --type process`

### Audit + Optimize Together -> /run improve

When the user wants both review and optimization with a single shared baseline:

**Examples**:
- "Audit src/ and apply safe optimizations with one baseline" -> `/run improve src/`
- "Review and tune the auth module" -> `/run improve src/auth/`
...
### Review-then-Fix
- "Find and fix security vulnerabilities" -> `/run review --focus security --auto-fix safe` (auto-fixes safe issues, then use `/run` for complex ones)

### Review + Optimize Together
- "Audit and tune the codebase with one baseline" -> `/run improve src/`
...
Could mean:
- **Optimize performance** -> `/run optimize src/login/ --type code`
- **Fix bugs** -> `/run Fix login page issues`
- **Redesign it** -> `/designer login page redesign`
- **Review quality** -> `/run review src/login/`

Present: "What kind of improvement? Performance (`/run optimize`), bug fixes (`/run`), redesign (`/designer`), or quality review (`/run review`)?"

### "Work on authentication"
Could mean:
- **Build it** -> `/run Implement authentication`
- **Plan it** -> `/designer authentication system`
- **Review it** -> `/run review src/auth/`
- **Optimize it** -> `/run optimize src/auth/`

Present: "What do you want to do with authentication? Build (`/run`), design (`/designer`), review (`/run review`), or optimize (`/run optimize`)?"
...
Based on your request: "improve the dashboard performance"
  ...
  Recommended: /run optimize src/dashboard/ --type code --focus performance

  Why: You want measurable performance improvements. /run optimize
  will establish baseline metrics, identify bottlenecks, apply optimizations
  atomically, and show you before/after metrics.

  Alternative: /run review src/dashboard/ --focus performance
  -- if you just want to identify issues without applying changes

  Ready to go? Just type:
    /run optimize src/dashboard/ --type code --focus performance
```
...
Based on your request: "work on the payment system"
  ...
  3. /run review src/payments/
     -- if you want to CHECK the existing payment code

  4. /run optimize src/payments/
