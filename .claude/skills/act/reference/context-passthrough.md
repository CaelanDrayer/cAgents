# /act context Passthrough (REMOVED in V11.0)

**Status: REMOVED.** The `/act context show|init|update|clear` passthrough no
...
`/act` no longer treats `context` as a reserved first positional token. If a
user types `/act context show`, /act will treat the entire string as a
...
for the unchanged read contract. Only the WRITE-side CLI (`/act context …`)
was removed.
...
|------------------------|--------------------|
| `/act context show`   | Print the current `product_context.yaml`              |
| `/act context init`   | Scan project, write initial `product_context.yaml`    |
| `/act context update` | Interactively merge updates into `product_context.yaml` |
| `/act context clear`  | Remove `product_context.yaml` (keep parent directory) |

These dispatched directly to `Skill({ skill: "context", args: <subcommand> })`
without entering the `/act` state machine. The dispatch was removed in V11.0
...
|----------------------|------------------------------------------------------------------|
| `/act context show`   | `cat cagents-memory/_projects/{hash}/product_context.yaml`      |
| `/act context init`   | Manually create the file using your project's known conventions |
| `/act context update` | Edit the file directly in your editor                           |
| `/act context clear`  | `rm cagents-memory/_projects/{hash}/product_context.yaml`       |
...
either moved into `/improve` (review/optimize) or `/act --mode debug`
...
- `.claude/skills/act/SKILL.md` — parent skill; the live passthrough block was removed
