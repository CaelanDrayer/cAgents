...
Parse the command arguments before you delegate:
...
|------|------|-------------|---------|---------|
| `--interactive` | Boolean | Enable the interactive mode. It asks the user for preferences before the run starts. | false | `/act Fix bug --interactive` |
| `--dry-run` | Boolean | Preview the workflow with no execution. It shows the plan and then stops. | false | `/act Add feature --dry-run` |
| `--quiet`, `-q` | Boolean | Skip the plan display, and go directly to the execution | false | `/act Fix bug --quiet` |
| `--stream` | Boolean | Give real-time progress updates during the execution | false | `/act Deploy app --stream` |
| `--skip-preflight` | Boolean | Skip the pre-flight validation. This flag is not recommended. | false | `/act Hotfix --skip-preflight` |
| `--template <name>` | String | Use a specific workflow template | auto-match | `/act Budget --template budget_creation` |
| `--domain <domain>` | String | Override the domain detection | auto-detect | `/act Analyze --domain engineering` |
| `--tier <N>` | Number | Override the tier classification, from 2 to 4 | auto-classify | `/act Migrate --tier 4` |
| `--confidence <N>` | Number | Set the detection confidence threshold | 0.7 | `/act Request --confidence 0.6` |
| `--team` | Boolean | Use the parallel team execution of Agent Teams | false | `/act Build feature --team` |
| `--resume <id>` | String | Resume an interrupted workflow session | none | `/act --resume act_fix-auth_260207_001` |
| `--session <dir>` | String | A pre-enriched session directory. The /team subagents use it. | none | `/act Implement feature --session cagents-memory/sessions/team_xyz_260512_001` |
...
| `--analytics` | Boolean | Display the pipeline analytics dashboard, and then exit | false | `/act --analytics` |
| `--analytics domain` | String | Show the per-domain analytics breakdown | none | `/act --analytics domain` |
...
By default, `/act` shows the workflow plan after the planning completes. This
behavior applies to a tier 2 workflow and above:
- Use `--quiet` to skip this display, and to run silently
- Use `--dry-run` to see the plan and then stop, with no execution
