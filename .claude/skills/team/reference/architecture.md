...
More waves provide:
- **Quality gates between phases**: catch the issues early, and not at the end
- **Clear dependency boundaries**: the consumers explicitly wait for the providers
- **Smaller, focused work units**: a higher success rate per subagent
- **Better coordination**: the lead validates each phase before it proceeds
- **Natural separation of concerns**: research before design, design before implementation, and implementation before testing
...
- **Eliminates fragile multi-agent dependency**: there is no risk that a planner step fails to produce the files
- **Faster startup**: the decomposition happens in one step, and not in a multi-agent chain
- **Each wave subagent spawns execution agents directly**: the full agent orchestration happens per work item, and it does not re-enter the pipeline
- **Simpler to execute reliably**: fewer sequential dependencies give fewer failure points
...
Teams are implicit since CC v2.1.178. The `TeamCreate`/`TeamDelete` tools were removed, so do not call them. The one remaining failure mode is creating tasks (TaskCreate) without spawning subagents (Agent tool) to execute them. The correct sequence is:
```
...
`/team` uses Claude Code's **implicit agent teams** for parallel execution. The `TeamCreate`/`TeamDelete` tools were removed in CC v2.1.178. A team forms implicitly when you spawn subagents. Cleanup is automatic at session end. Do NOT call them.
...
Removed (do not call): `TeamCreate`, `TeamDelete`. CC v2.1.178 dropped both tools. Teams are implicit now, and cleanup is automatic.
...
Wave subagents NEVER implement work directly. They always coordinate through execution agents via the Agent tool. A subagent may need a different specialty. It spawns that specialist as its own downward subagent. The same 5-level ceiling still applies. That is how parallelism compounds beyond the per-wave fan-out.

**Why no Skill("act") fork**: Wave subagents are full Claude Code sessions. As of CC 2.1.172, a wave subagent can nest an `/act` call within the 5-level depth budget. The skill still avoids that call **by design for cost and clarity**. Invoking `/act` via Skill re-runs the full pipeline (orchestrator + planner + controller + validator). The extra run duplicates the Wave 0 enrichment the lead already did, and it burns extra context and tokens. Spawning execution agents directly keeps the chain short (subagent-controller -> execution agents -> reviewer) and reuses the lead's enrichment.
