...
- **With validator**: After execution phase, validator re-runs quality gates and produces PASS/FAIL/REVISE; optimizer handles rollback on FAIL
- **With self-correct**: When an optimization execution fails (tool error, test regression), self-correct is invoked to attempt recovery before rollback is triggered
