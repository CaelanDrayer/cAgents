> Mode `standards-audit` of `qa-lead` — relocated verbatim from `agents/developer/quality/code-standards-auditor/SKILL.md` (zero-loss consolidation).
...
See @resources/standards-audit-naming-conventions.md for language-specific naming rules.
See @resources/standards-audit-code-quality-checks.md for quality metrics and thresholds.
