...
- **Fallback Gracefully**: If the request produces fewer than 3 work items or all items are sequential, delegate to /act instead of creating a team with no parallelism benefit
...
- **Parent Session Linkage**: Each teammate's /act invocation creates a child session — link child sessions back to the parent team session via child_sessions.yaml for cross-session traceability
...
- **No Fallback Check**: Attempting team mode for requests with <3 work items or all-sequential dependencies — no parallelism benefit; fall back to /act
...
- **Fallback Rate**: How often the `/team` skill loop decides to fall back to /act — should be rare; high rates suggest incorrect suitability assessment
...
- **With orchestrator**: In /act --team mode, orchestrator detects the team flag and spawns the `/team` lead (formerly the team-trigger agent) instead of the domain controller — the `/team` loop handles all subsequent team coordination
