> Mode `state` of `task-state` — relocated verbatim from `agents/core/task-state/SKILL.md` (zero-loss consolidation).
...
See @resources/state-inventory-patterns.md for batch delegation and checkpoint patterns.

See @resources/state-best-practices.md for design principles and quality indicators.
