...
- **PASS/FAIL/REVISE/PARTIAL_PASS Classification**: PASS (all gates pass, all criteria met with evidence) → PARTIAL_PASS (most pass, some dead-letter items) → FAIL (fixable issues, re-run controller with feedback) → REVISE (fundamental planning issues, re-plan with feedback) — maps to the /act state machine routing
