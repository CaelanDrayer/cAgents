...
- **/act Skill Invocation in Teammates**: Instructing teammates to call Skill({skill: "act"}) to execute their work items — re-entering the full /act pipeline is wasteful; spawn execution agents directly via Agent (cheaper and clearer, and well within the 5-level nesting budget)
