...
_V11.0 removed /review, /optimize, /context, /debug — see [MIGRATION-V11.md](./MIGRATION-V11.md). v12.1.2 folded /improve into /act via a first-word keyword router. v12.2.0 removed /org and absorbed it into /team strategic mode._

This decision matrix routes review and optimization work to `/act review|optimize|improve <target>` (v12.1.2+ keyword router; or `/act --mode review|optimize|full`), product context to `/act context <subcmd>`, systematic debugging to `/act --mode debug`, and cross-domain coordination to `/team --strategic` (v12.2.0+; auto-enabled when `router.domain_count >= 2`).
  ...
  NO  -> Is the work parallelizable with 3+ independent items?
           ...
           NO  -> Is this an interactive design session?
                    ...
                    NO  -> Is this a review of existing code/content?
                             YES -> /act review <target>  (or /act --mode review)
                             NO  -> Is this an optimization of existing work?
                                      YES -> /act optimize <target>  (or /act --mode optimize)
                                      NO  -> Want both review and optimize with one baseline?
                                               YES -> /act improve <target>  (or /act --mode full)
                                               NO  -> /act
```

## Decision Matrix

| Criteria | /act (incl. review/optimize/improve modes) | /team (incl. strategic mode) | /designer | /helper |
|----------|--------------------------------------------|-------------------------------|-----------|---------|
...
- **Passthroughs**: `/act context show|init|update|clear` (product context), `/act --mode debug <bug>` (systematic debugging)
...
- **Falls back to**: /act if <3 items or no parallelism
...
- **Avoid when**: Work fits in a single domain (use /act or flat /team instead)
...
### /act review|optimize|improve - Review + Optimization Modes (v12.1.2+; replaces standalone /improve)
- **Best for**: Reviewing existing code/content, applying optimizations with metrics, or both with a shared baseline
- **Examples**:
  - `/act review src/auth/` — audit auth module for security/quality issues (= `--mode review`)
  - `/act optimize src/api/` — reduce response times with before/after benchmarks (= `--mode optimize`)
  - `/act improve src/checkout/` — review then optimize with one baseline (= `--mode full`)
- **Characteristics**: Runs through `/act`'s standard 5-state machine (INIT → ORCHESTRATED → PLANNED → COORDINATED → VALIDATED); the improve phases (scoping, measuring, detecting, planning, executing, validating, reporting) are carried controller-internally during the PLANNED state rather than as a distinct state machine. Atomic rollback, pattern-effectiveness tracking, unified `improve_report.md`
- **Mode selection**: keyword router (first word `review|audit|optimize|improve`) or explicit `--mode review|optimize|full`
- **Migration**: Pre-v12.1.2 this was the standalone `/improve` skill, which was folded into `/act` via the keyword router
- **Avoid when**: Creating new work (use plain `/act`) or designing before building (use `/designer`)
...
|------|-------------|-------------------|
| 2 (Moderate) | Single component, clear scope | /act |
| 3 (Complex) | Multiple components, external deps | /team or /act |
...
| Flag | Skills | Effect |
|------|--------|--------|
| `--dry-run` | /act (incl. review/optimize/full modes), /team (incl. strategic mode) | Preview plan without executing |
| `--team` | /act | Force team mode from /act |
| `--waves N` | /team | Set minimum wave count |
| `--members N` | /team | Max teammates per wave |
| `--analytics` | /act | Show execution analytics |
| `--mode review\|optimize\|full` | /act | Select review, optimize, or chained mode (v12.1.2+: `/act review|optimize|improve` keyword router selects automatically) |
| `--mode debug` | /act | Systematic 4-phase debugging passthrough |
| `--scope <path>` | /act | Restrict scope to a path (for review/optimize modes) |
| `--baseline` | /act | Establish review baseline (for review/optimize modes) |
| `--suppress` | /act | Suppress baselined findings (for review/optimize modes) |
| `--benchmark` | /act | Capture before/after benchmark numbers (for optimize/full modes) |
| `--focus <area>` | /designer, /act | Focus on a specific area (for review/optimize modes on /act) |
