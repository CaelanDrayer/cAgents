...
60 agents across 9 archetypes — 44 routable + 16 core — each with a single SKILL.md scoped to one role. This is the post-v12.20.0 catalog consolidation from 141: the 88 absorbed agents were collapsed into mode flags rather than separate SKILL.md files (disk-derived: 88 distinct `absorbed from X` markers across the 32 multi-mode SKILL.md files), and 0 `_deprecated/` SKILL.md remain on disk. The `skill-size-monitor.cjs` `PreToolUse` hook warns at 600 lines and blocks at 900 to prevent SKILL.md bloat. The catalog may be over-fragmented for some domains — a future consolidation pass remains on the roadmap (see `CHANGELOG.md` for the v12.0.0/v12.7.0 consolidation pattern, which the next pass would extend) — but the "small, focused" principle is honored.
