...
_V11.0 removed /review, /optimize, /context, /debug — see [MIGRATION-V11.md](./MIGRATION-V11.md). v12.1.2 folded /improve into /act via a first-word keyword router; review and optimization are now modes of `/act` (`/act review`, `/act optimize`, `/act improve` — or `--mode review|optimize|full`). v12.2.0 removed /org and absorbed cross-domain coordination into `/team` strategic mode (auto-enabled when `router.domain_count >= 2`)._
...
**Routing**: 1 domain + simple scope -> /act. 1 domain + complex -> flat /team. 2+ domains -> /team strategic mode (Wave 0/1/2 C-suite deliberation + Wave 3..N per-domain dispatch in the same session).
...
```
/act (state machine, level 0; 5-state pipeline since v12.0.0)
...
**Example** (`/act Fix login auth bug`):
...
### /act review (or --mode review) -- Parallel Review (v12.1.2+; replaces standalone /improve --mode review)
...
### /act optimize (or --mode optimize) -- 5-Phase Optimization (v12.1.2+; replaces standalone /improve --mode optimize)
...
### /act improve (or --mode full) -- Combined Review + Optimize (v12.1.2+; replaces standalone /improve --mode full)
...
Build offer: /act, /team, save, or continue refining
```
...
/designer -> /act                      Design then build
/designer -> /team                     Design then parallel build
/act review <target> -> /act <fix>     Find issues then fix (v12.1.2+ keyword router)
/act optimize -> /act review           Improve then verify
/act improve <target>                  Single-pass review + optimize synthesis (= --mode full)
/act --team                            Shortcut for parallel execution
/team (fallback) -> /act               Auto-delegates if <3 work items
```
