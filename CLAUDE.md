...
- Bug fix/minor tweak: patch (9.10.0 -> 9.10.6)
- New feature/enhancement: minor (9.10.6 -> 9.11.0)
...
**Architecture**: CJS-only hooks with `createHook()` factory. All invoked via `node "$CLAUDE_PROJECT_DIR"/.claude/hooks/run-hook.cjs <hook-name>` for resilient path resolution. See @.claude/rules/core/hooks.md for full documentation.
