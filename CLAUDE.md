...
|   +-- hooks/               # 32 .cjs files (26 registered hooks + 3 dispatched sub-validators + utils + launcher + eval CLI)
...
**Architecture**: CJS-only hooks with `createHook()` factory. 32 .cjs files = 26 unique registered hooks + 3 dispatched Write|Edit sub-validators (secret-detection, controller-delegation-validator, skill-size-monitor — run in-process by `write-edit-dispatch.cjs`, the D1b consolidating dispatcher; v12.19.0 WI-5) + hook-utils.cjs + run-hook.cjs launcher + eval-runner.cjs CLI. See @.claude/rules/core/hooks.md for full documentation.
...
**Tests**: `npm test` runs 1315+ Vitest tests across 154+ files (hooks + config validation + regression tests; static lower-bound — actual runtime count is higher because `it.each` rows expand to multiple tests)
**Version**: 12.19.0
