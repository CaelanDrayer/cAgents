...
After writing plan.yaml + work_items.yaml + (when waves are defined) work_meta.yaml + per-wave files (and optionally delegation_prompts.yaml for tier 3+), write a completion event to `workflow/events/`:
...
outputs_produced:
  ...
  # when waves are defined (typical under /team or /run --waves N):
  - workflow/work_meta.yaml
  - workflow/work_items_wave_0.yaml
  - workflow/work_items_wave_1.yaml
  # ... one per wave
  # optional, tier 3+:
