// Regression test for V10.26.9 — /act context passthrough subcommands
// Asserts the documentation contract for /act context show|init|update|clear.
//
// V11.0 update: /context skill was removed in V11.0 (see docs/MIGRATION-V11.md).
// The /act context passthrough no longer dispatches to a sibling skill.
...
const PASSTHROUGH_REF = resolve(
  process.cwd(),
  '.claude/skills/act/reference/context-passthrough.md'
);

describe('V10.26.9 /act context passthrough', () => {
  ...
  it('reference documents all four subcommand patterns', () => {
    for (const sub of ['show', 'init', 'update', 'clear']) {
      expect(refContent).toMatch(new RegExp(`/act context ${sub}`));
    }
  });
  ...
  it('act/SKILL.md acknowledges the V11.0 /context skill removal', () => {
  ...
  it('act/SKILL.md documents the historical subcommand syntax', () => {
    // The combined form `/act context show|init|update|clear` must remain in
    ...
    expect(runSkillContent).toMatch(
      /\/act context show\|init\|update\|clear/
    );
  });

  it('act/SKILL.md points users at product_context.yaml for direct edits', () => {
    ...
    // directly since no /act subcommand wraps it anymore.
    expect(runSkillContent).toMatch(/product_context\.yaml/);
  });

  it('act/SKILL.md references the passthrough reference doc', () => {
  ...
  it('routing would dispatch to /context rather than run the full pipeline', () => {
    ...
    const tokens = '/act context show'.split(/\s+/).slice(1); // drop the command
    const isPassthrough =
      tokens.length >= 2 &&
      tokens[0] === 'context' &&
  ...
  it('non-passthrough requests fall through to the state machine', () => {
    const tokens = '/act fix auth bug'.split(/\s+/).slice(1);
    const isPassthrough =
      tokens.length >= 2 &&
      tokens[0] === 'context' &&
      ['show', 'init', 'update', 'clear'].includes(tokens[1]);
